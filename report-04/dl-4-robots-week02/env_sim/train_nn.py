...
def save_model(model):
    torch.save(model.state_dict(), 'saved_models/mysavedmodel.pth')
...
def train_model(model, epoch, data, label, optimizer, batch_sz):
    #Prepare model for minibatch training
    model.train()
    optimizer.zero_grad()
    train_loss = 0
    train_step_counter = 0
    mseloss = nn.MSELoss() #Define loss function
    ...
    for batch_idx in range(int(data.shape[0]/batch_sz)):
        ...
        #calculate dloss/dx and update weights
        loss.backward()
        optimizer.step()

    print('Train Epoch: {} \tLoss: {:.6f}'.format(epoch, train_loss.cpu().numpy()/train_step_counter))


'''Test Neural Network'''
def test_model(model, data, label, batch_sz):
...
def runTrain(epochs=500):
    ...
    train_file = io.BytesIO(pkgutil.get_data(__name__, 'data/train.npy'))
    test_file = io.BytesIO(pkgutil.get_data(__name__, 'data/test.npy'))
    ...
    for epoch in range(epochs):
        ...
        train_model(model, epoch, data, label, optimizer, batch_sz)
        if epoch % 1 == 0 and epoch != 0:
            with torch.no_grad():
                current_loss = test_model(model, test_data, test_label, batch_sz)
            if current_loss < best_loss:
                best_loss = current_loss

    with torch.no_grad():
        test_model(model, test_data, test_label, batch_sz)
