...
class GRUCell(nn.Module):
    ...
    def reset(self, batch_size=1, cuda=True):
        ...
        #Reset recurrent states (single recurrent state h_t)
        if cuda:
            self.recurrent_state = torch.zeros((batch_size, self.hidden_size), dtype=torch.float64, device=torch.device('cuda:0'), requires_grad=True)
        else:
            self.recurrent_state = torch.zeros((batch_size, self.hidden_size), dtype=torch.float64, device=torch.device('cpu'), requires_grad=True)
        #########################################################################
