...
class LSTMCell(nn.Module):
    ...
    def forward(self, X_t):
        ...
        f_t = torch.sigmoid(torch.mm(X_t, self.W_f) + 
                            torch.mm(h_t_previous, self.U_f) + self.b_f)

        i_t = torch.sigmoid(torch.mm(X_t, self.W_i) + 
                            torch.mm(h_t_previous, self.U_i) + self.b_i)

        o_t = torch.sigmoid(torch.mm(X_t, self.W_o) + 
                            torch.mm(h_t_previous, self.U_o) + self.b_o)

        candidate = torch.tanh(torch.mm(X_t, self.W_c) + 
                            torch.mm(h_t_previous, self.U_c) + self.b_c)

        c_t = f_t*c_t_previous + i_t*candidate
