...
class BotEnv:
    def __init__(self, model, screen, clock):
        #Pygame
        self.screen = screen
        self.clock = clock
    ...
    def DrawEverything(self,flag=0):
        ...
        self.screen.blit(img,to_pygame(AdjustedImagePosition,self.screen))

        if(flag==0 and self.DetectCrash == 0):
            (self.BotRect.x,self.BotRect.y) = self.Bot.position[0],600-self.Bot.position[1]
            self.CircleRect = pygame.draw.circle(self.screen, (169,169,169), (self.BotRect.x,self.BotRect.y), 20, 0)
        ## If Collision Detected Draw Green
        elif(flag==0 and self.DetectCrash >= 1):
            (self.BotRect.x,self.BotRect.y) = self.Bot.position[0],600-self.Bot.position[1]
            self.CircleRect = pygame.draw.circle(self.screen, (0,255,0), (self.BotRect.x,self.BotRect.y), 20, 0)
        ## If Collision Detected Draw Red
        else:
            (self.BotRect.x,self.BotRect.y) = self.Bot.position[0],600-self.Bot.position[1]
            self.CircleRect = pygame.draw.circle(self.screen, (255,0,0), (self.BotRect.x,self.BotRect.y), 20, 0)
        ...
        self.screen.blit(img,AdjustedImagePosition)
        for ob in self.WallRects:
            pygame.draw.rect(self.screen, (169,169, 169), ob)
    ...
    def _step(self, action, CrashStep=0):
        ...
        self.clock.tick(ClockTickValue)    ## Tick The Clock
        ...
        DataTensor = tf.constant(SensorsData[:-1])
        DataTensor = np.expand_dims(DataTensor,0)
        if (self.model != None):
            ## Get Decision From Neural Network If There Is A Collison
            self.DetectCrash = self.model(DataTensor)
            self.DetectCrash = np.round(self.DetectCrash)
    ...
    def RecoverFromCrash(self, BotDirection):
        ...
        while self.crashed:
            ...
            for i in range(1):
                ...
                pygame.display.flip()
                self.clock.tick(ClockTickValue)
            ...
            if PixelInGame[0] <= 0 or PixelInGame[1] <= 0 or PixelInGame[0] >= width or PixelInGame[1] >= height:
                ...
                for ob in self.WallRects:
                    if ob.collidepoint((PixelInGame[0],PixelInGame[1])):
                        return distance
        ## Draw The Sensor
        pygame.draw.line(self.screen,(30,144,255),SensorStartInGame,SensorEndInGame)
        return distance
...
def runSim(model, loc=None):
    ...
    if loc in [3, 1, 4]:
        BotStartLocation = loc

    env = BotEnv(model, screen, clock)
    random.seed(10)
    env._step(5)
    for i in range(2000):
